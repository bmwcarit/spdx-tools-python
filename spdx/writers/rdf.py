...
from tagvalue import InvalidDocumentError
from rdflib import *
from .. import file
...
class BaseWriter(object):
    ...
    def create_checksum_node(self, chksum):
        ...
        algorithm_triple = (chksum_node, self.spdx_namespace.algorithm,
                            Literal(chksum.identifier))
        self.graph.add(algorithm_triple)
        value_triple = (chksum_node, self.spdx_namespace.checksumValue,
...
class LicenseWriter(BaseWriter):
    ...
    def create_license_helper(self, license):
        """Handles single(no conjunction/disjunction) licenses.
        Returns the created node.
        """
        if isinstance(license, document.ExtractedLicense):
            return self.create_extracted_license(license)
        elif license.identifier in config.LICENSE_MAP:
            return URIRef(license.url)
        else:
            matches = [l for l in self.document.extracted_licenses if
                       l.identifier == license.identifier]
            if len(matches) != 0:
                return self.create_extracted_license(matches[0])
            else:
                raise InvalidDocumentError(
                    'Missing extracted license: {0}'.format(license.identifier))

    def create_extracted_license(self, license):
        """Handles extracted license.
        Returns the license node.
        """
        licenses = list(self.graph.triples((
            None, self.spdx_namespace.licenseId, license.identifier)))
        if len(licenses) != 0:
            return licenses[0][0]  # return subject in first triple
        else:
            license_node = BNode()
            type_triple = (license_node, RDF.type,
                           self.spdx_namespace.ExtractedLicensingInfo)
            self.graph.add(type_triple)
            ident_triple = (license_node, self.spdx_namespace.licenseId,
                            Literal(license.identifier))
            self.graph.add(ident_triple)
            text_triple = (license_node, self.spdx_namespace.extractedText,
                           Literal(license.text))
            self.graph.add(text_triple)
            if license.full_name is not None:
                name_triple = (license_node, self.spdx_namespace.licenseName,
                               self.to_special_value(license.full_name))
                self.graph.add(name_triple)
            for ref in license.cross_ref:
                triple = (license_node, RDFS.seeAlso, URIRef(ref))
                self.graph.add(triple)
            if license.comment is not None:
                comment_triple = (license_node, RDFS.comment,
                                  Literal(license.comment))
                self.graph.add(comment_triple)
            return license_node

    def create_license_node(self, license):
        ...
        if isinstance(license, document.LicenseConjunction):
            ...
            return self.create_license_helper(license)

    def license_or_special(self, license):
        ...
        """
        if isinstance(license, utils.NoAssert):
            return self.spdx_namespace.noassertion
        elif isinstance(license, utils.SPDXNone):
            return self.spdx_namespace.none
        else:
            return self.create_license_node(license)
...
class FileWriter(LicenseWriter):
    ...
    def create_file_node(self, doc_file):
        ...
        name_triple = (file_node, self.spdx_namespace.fileName,
                       Literal(doc_file.name))
        self.graph.add(name_triple)

        if doc_file.has_optional_field('comment'):
            comment_triple = (file_node, RDFS.comment,
                              Literal(doc_file.comment))
            self.graph.add(comment_triple)
        ...
        self.graph.add((file_node, self.spdx_namespace.checksum,
                        self.create_checksum_node(doc_file.chk_sum)))

        conc_lic_node = self.license_or_special(doc_file.conc_lics)
        conc_lic_triple = (file_node, self.spdx_namespace.licenseConcluded,
                           conc_lic_node)
        self.graph.add(conc_lic_triple)

        license_info_nodes = map(
            self.license_or_special, doc_file.licenses_in_file)
        for license in license_info_nodes:
            triple = (file_node, self.spdx_namespace.licenseInfoInFile,
                      license)
            self.graph.add(triple)

        if doc_file.has_optional_field('license_comment'):
            comment_triple = (file_node, self.spdx_namespace.licenseComments,
                              Literal(doc_file.license_comment))
            self.graph.add(comment_triple)

        cr_text_node = self.to_special_value(doc_file.copyright)
        cr_text_triple = (file_node, self.spdx_namespace.copyrightText,
                          cr_text_node)
        self.graph.add(cr_text_triple)

        if doc_file.has_optional_field('notice'):
            notice_triple = (file_node, self.spdx_namespace.noticeText,
                             doc_file.notice)
            self.graph.add(notice_triple)

        contrib_nodes = map(lambda c: Literal(c), doc_file.contributers)
        contrib_triples = [(file_node, self.spdx_namespace.fileContributor,
                            node) for node in contrib_nodes]
    ...
    def add_file_dependencies_helper(self, doc_file):
        ...
        subj_triples = list(self.graph.triples(
            (None, self.spdx_namespace.fileName, Literal(doc_file.name))))
        if len(subj_triples) != 1:
            raise InvalidDocumentError('Could not find dependency subject {0}'
                                       .format(doc_file.name))
        subject_node = subj_triples[0][0]
        for dependency in doc_file.dependencies:
            dep_triples = list(self.graph.triples(
                (None, self.spdx_namespace.fileName, Literal(dependency))))
            if len(dep_triples) == 1:
                dep_node = dep_triples[0][0]
                dep_triple = (subject_node, self.spdx_namespace.fileDependency,
                              dep_node)
                self.graph.add(dep_triple)
            else:
                print 'Warning could not resolve file dependency {0} -> {1}'.format(
...
class ReviewInfoWriter(BaseWriter):
    ...
    def create_review_node(self, review):
        ...
        self.graph.add((review_node,
                        self.spdx_namespace.reviewer, reviewer_node))
        reviewed_date_node = Literal(review.review_date_iso_format)
        reviewed_triple = (review_node,
                           self.spdx_namespace.reviewDate, reviewed_date_node)
...
class PackageWriter(LicenseWriter):
    ...
    def package_verif_node(self, package):
        ...
        verif_node = BNode()
        type_triple = (verif_node, RDF.type,
                       self.spdx_namespace.PackageVerificationCode)
        self.graph.add(type_triple)
        value_triple = (verif_node,
        ...
        excl_file_triples = [(verif_node, excl_predicate, xcl_file)
                             for xcl_file in excl_file_nodes]
        for trp in excl_file_triples:
            self.graph.add(trp)
        return verif_node

    def handle_package_literal_optional(self, package,
                                        package_node, predicate, field):
    ...
    def handle_pkg_optional_fields(self, package, package_node):
        ...
        self.handle_package_literal_optional(package, package_node,
                                             self.spdx_namespace.versionInfo, 'version')

        self.handle_package_literal_optional(package, package_node,
                                             self.spdx_namespace.packageFileName, 'file_name')

        self.handle_package_literal_optional(package, package_node,
                                             self.spdx_namespace.supplier, 'supplier')

        self.handle_package_literal_optional(package, package_node,
                                             self.spdx_namespace.originator, 'originator')

        self.handle_package_literal_optional(package, package_node,
                                             self.spdx_namespace.sourceInfo, 'source_info')

        self.handle_package_literal_optional(package, package_node,
                                             self.spdx_namespace.licenseComments, 'license_comment')

        self.handle_package_literal_optional(package, package_node,
                                             self.spdx_namespace.summary, 'summary')

        self.handle_package_literal_optional(package, package_node,
                                             self.spdx_namespace.description, 'description')

        if package.has_optional_field('check_sum'):
            checksum_node = self.create_checksum_node(package.check_sum)
            self.graph.add((package_node, self.spdx_namespace.checksum,
                            checksum_node))

        if package.has_optional_field('homepage'):
            homepage_node = URIRef(self.to_special_value(package.homepage))
            homepage_triple = (package_node, self.doap_namespace.homepage,
                               homepage_node)
    ...
    def create_package_node(self, package):
        ...
        # package name
        name_triple = (
            package_node, self.spdx_namespace.name, Literal(package.name))
        self.graph.add(name_triple)
        # Package download location
        down_loc_node = (package_node, self.spdx_namespace.downloadLocation,
        ...
        verif_triple = (package_node, self.spdx_namespace.packageVerificationCode,
                        verif_node)
        self.graph.add(verif_triple)
        # Handle concluded license
        conc_lic_node = self.license_or_special(package.conc_lics)
        conc_lic_triple = (package_node, self.spdx_namespace.licenseConcluded,
                           conc_lic_node)
        self.graph.add(conc_lic_triple)
        # Handle declared license
        decl_lic_node = self.license_or_special(package.license_declared)
        decl_lic_triple = (package_node, self.spdx_namespace.licenseDeclared,
                           decl_lic_node)
        self.graph.add(decl_lic_triple)
        # Package licenses from files
        licenses_from_files_nodes = map(lambda el: self.license_or_special(el),
                                        package.licenses_from_files)
        lic_from_files_predicate = self.spdx_namespace.licenseInfoFromFiles
        lic_from_files_triples = [(package_node,
                                   lic_from_files_predicate, node) for node in licenses_from_files_nodes]
        for triple in lic_from_files_triples:
            self.graph.add(triple)
        # Copyright Text
        cr_text_node = self.to_special_value(package.cr_text)
        cr_text_triple = (package_node, self.spdx_namespace.copyrightText,
                          cr_text_node)
        self.graph.add(cr_text_triple)
        # Handle files
    ...
    def handle_package_has_file_helper(self, pkg_file):
        ...
        """
        nodes = list(self.graph.triples((None,
                                         self.spdx_namespace.fileName, Literal(pkg_file.name))))
        if len(nodes) == 1:
            return nodes[0][0]
        else:
    ...
    def handle_package_has_file(self, package, package_node):
        ...
        triples = [(package_node, self.spdx_namespace.hasFile, node)
...
class Writer(CreationInfoWriter, ReviewInfoWriter, FileWriter, PackageWriter):
    ...
    def create_doc(self):
        ...
        # Version
        vers_literal = Literal('SPDX-{0}.{1}'.format(
            self.document.version.major, self.document.version.minor))
        self.graph.add((doc_node, self.spdx_namespace.specVersion,
                        vers_literal))
        # Data license
    ...
    def write(self):
        ...
        ci_triple = (
            doc_node, self.spdx_namespace.creationInfo, creation_info_node)
        self.graph.add(ci_triple)
        # Add review info
        review_nodes = self.reviews()
        for review in review_nodes:
            self.graph.add((
                doc_node, self.spdx_namespace.reviewed, review))
        # Add extracted licenses
        licenses = map(
            self.create_extracted_license, self.document.extracted_licenses)
        for license in licenses:
            self.graph.add((doc_node,
                            self.spdx_namespace.hasExtractedLicensingInfo, license))
        # Add files
        files = self.files()
